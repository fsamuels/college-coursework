//: String.cpp (reference count version) - implementation of an abstract data
//                 type String (adapted from "Learning C++" by Neill Graham)

// Matt Stallmann
// (prehistoric; last update 21 Apr 2000)
...
String::String(int n)
{
  ostrstream out_string;
  out_string << n;
  const char * stg = out_string.str();
  bufferlength = CstringLength(stg);
  buffer = new char[bufferlength + 2];
  assert(buffer != 0);
  for (int i = 0; i < bufferlength; i++)
  ...
  buffer[bufferlength+1] = '\0';
  *buffer = 1;
  out_string.rdbuf()->freeze(0); // make sure memory is properly deallocated
}
