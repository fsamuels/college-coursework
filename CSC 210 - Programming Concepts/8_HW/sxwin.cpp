...
// Matt Stallmann, ca. 1995
// 02 Feb 1999 - added exception handling to avoid segmentation fault when
//               display is not available (MS)
// 15 Apr 1999 - added a cursorPosition method to query current cursor
//               position (Christopher G. Healey)
// 22 Mar 2000 - added recognition of arrow keys with appropriate return
//               values (Chris Church) 
// 25 Apr 2000 - added drawText to draw multiple lines of text with
//               appropriate spacing (MS)
...
sxwin::sxwin(int x, int y, int width, int height, const String title) {
  ...
  assert( data_->display_ && "X11 error: unable to open default display." );
      ...
      case KeyPress:
        { char ans[1];			//for XLookupString
          ...
          if (key >= 32 && key <= 126) { // printable ASCII character
            ...
            // Chris's Code (22 Mar 2000)
            else if (key == XK_Up) c = UP_ARROW;
            else if (key == XK_Down) c = DOWN_ARROW;
            else if (key == XK_Left) c = LEFT_ARROW;
            else if (key == XK_Right) c = RIGHT_ARROW;
            // End of Chris's Code
            if (c != 0) {
              type = KeyStroke;
              return;
...
void sxwin::drawText(int x, int y, String text[], int num_lines,
                     int padding, double spacing)
{
  XFontStruct* fontPtr = XQueryFont(data_->display_, data_->font_);
  int dummy;
  XCharStruct dummyCS;
  int font_ascent;
  int font_descent;
  XTextExtents(fontPtr, "", 0, &dummy, &font_ascent, &font_descent,
               &dummyCS);
  int line_height = int( spacing * (font_ascent + font_descent));
  int total_height = line_height * num_lines + 2 * padding;
  unsigned int maximum_width = 0;
  for ( int i = 0; i < num_lines; ++i ) {
    unsigned int string_width = stringWidth( text[i] );
    if ( string_width > maximum_width ) maximum_width = string_width;
  }
  int total_width = maximum_width + 2 * padding;
  clearArea( x, y, total_width, total_height );

  for ( int i = 0; i < num_lines; ++i )
    drawString( x + padding,
                y + i * line_height + int( spacing * font_ascent ) + padding,
                text[i] );
}

void sxwin::textSize(int & width, int & height, String text[], int num_lines,
                     int padding, double spacing) const
{
  XFontStruct* fontPtr = XQueryFont(data_->display_, data_->font_);
  int dummy;
  XCharStruct dummyCS;
  ...
  XTextExtents(fontPtr, "", 0, &dummy, &font_ascent, &font_descent,
               &dummyCS);
  int line_height = int( spacing * (font_ascent + font_descent));
  height = line_height * num_lines + 2 * padding;
  unsigned int maximum_width = 0;
  for ( int i = 0; i < num_lines; ++i ) {
    unsigned int string_width = stringWidth( text[i] );
    if ( string_width > maximum_width ) maximum_width = string_width;
  }
  width = maximum_width + 2 * padding;
}
